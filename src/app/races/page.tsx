...
function MiniTrackMap({ slug }: { slug: string }) {
  ...
  return (
    <svg viewBox="0 0 200 150" className="w-12 h-12">
      <path d={path} fill="none" stroke="#FCD116" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" />
      {start && <circle cx={start.x} cy={start.y} r="6" fill="#CE1126" />}
    </svg>
  );
}
    ...
    <div className="bg-racing-black text-white">
      ...
      <div className="border-b border-antigua-gold/20 px-4 py-3 md:px-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 font-bold">SEASON 2 • 8 ROUNDS</p>
        </div>
                ...
                <div className="flex items-stretch">
                  ...
                  <div className="flex-1 px-3 py-2.5 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-sm font-bold truncate">{raceData?.track || 'TBA'}</h3>
                      {isCompleted ? (
